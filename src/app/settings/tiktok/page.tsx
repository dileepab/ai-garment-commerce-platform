...
export default async function TikTokSettingsPage({
  ...
  const views = await Promise.all(brandNames.map(getTikTokConnectionView));
  ...
  return (
    <main className="main">
      <PageHeader
        title="TikTok Ads"
        subtitle="Secure Marketing API authorization, advertiser assignment, and connection health"
        actions={
      ...
      <div className="content" style={{ display: 'grid', gap: 18 }}>
        ...
        <section style={{ ...cardStyle, background: '#F7F6F2' }}>
          <strong style={{ fontSize: 13, color: 'var(--color-fg-1)' }}>Marketing API boundary</strong>
          <p className="app-muted" style={{ marginTop: 6, lineHeight: 1.55 }}>
            This secure connection is the foundation for advertiser accounts, campaigns, reporting, creatives, and product catalogs.
            Live feature calls can be enabled after TikTok approval and account authorization are verified. Organic posting,
            comments, direct messages, and TikTok Shop onboarding require separate TikTok products or approvals.
          </p>
        </section>
        ...
        {views.map((view, index) => {
          ...
          return (
            <section key={view.brand} style={cardStyle}>
