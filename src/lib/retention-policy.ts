...
export function shouldSendCartRecoveryReminder(params: {
  ...
  if (params.now.getTime() - params.stateUpdatedAt.getTime() < CART_RECOVERY_DELAY_MS) {
    return block('not_stale_enough');
  }

  if (
    params.recentSentAt &&
    params.now.getTime() - params.recentSentAt.getTime() < CART_RECOVERY_COOLDOWN_MS
  ) {
...
export function shouldSendSupportTimeoutFollowUp(params: {
  ...
  if (params.now.getTime() - params.escalationUpdatedAt.getTime() < SUPPORT_TIMEOUT_DELAY_MS) {
    return block('support_case_not_stale_enough');
  }

  if (
    params.recentSentAt &&
    params.now.getTime() - params.recentSentAt.getTime() < SUPPORT_TIMEOUT_COOLDOWN_MS
  ) {
...
export function shouldSendPurchaseRetentionMessage(params: {
  ...
  if (
    params.recentSentAt &&
    params.now.getTime() - params.recentSentAt.getTime() < PURCHASE_NUDGE_COOLDOWN_MS
  ) {
