...
const PRODUCT_QUESTION_TYPES = ['colors', 'sizes', 'price', 'availability'] as const;
const PAYMENT_METHODS = ['COD', 'Online Transfer'] as const;
const PRODUCT_TYPES = ['tops', 'dresses', 'pants', 'skirts'] as const;
...
interface RouterProductContext {
  name: string;
  style: string;
  price: number;
  sizes: string;
...
const SIZE_PATTERN = /\b(XXL|XL|XS|S|M|L|small|medium|large|extra small|extra large)\b/i;
...
function scoreProductMatch(product: RouterProductContext, text: string): number {
...
function normalizeSize(size: string | null, product?: RouterProductContext | null): string | null {
  if (!size) {
    return null;
  }
...
function extractSizeFromMessage(message: string, product?: RouterProductContext | null): string | null {
  const match = message.match(SIZE_PATTERN);
  return match?.[1] ? normalizeSize(match[1], product) : null;
}

function extractColorFromMessage(message: string, product?: RouterProductContext | null): string | null {
  if (!product) {
    return null;
  }
...
function inferProductType(message: string): SizeChartCategory | null {
