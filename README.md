...
4. Deploy the database migration, then open **Settings → TikTok**.
...
## TikTok Support Inbox Setup

TikTok comments and direct messages use a separate TikTok Business Account
authorization. The advertiser token above cannot be reused. GarmentOS stores
the short-lived access token and rotating refresh token encrypted per brand.

Before enabling the connection:

1. Request TikTok Accounts permissions for `comment.list`,
   `comment.list.manage`, and `video.list`.
2. Request Business Messaging permissions for `message.list.read`,
   `message.list.send`, `message.list.manage`, and `user.account.type`, then
   complete TikTok's Data Security & Privacy Review.
3. In the TikTok developer app, register the exact account callback
   `https://app.deez.lk/api/integrations/tiktok/account/callback/`. TikTok
   requires this callback to end with `/`.
4. Copy the exact account-holder authorization URL generated by TikTok into
   `TIKTOK_ACCOUNT_AUTHORIZATION_URL`, and set
   `TIKTOK_ACCOUNT_REDIRECT_URI` and `TIKTOK_WEBHOOK_CALLBACK_URL` as shown in
   `.env.example`.
5. Deploy the `TikTokAccountConnection` and `TikTokInboxContext` migration.
6. Open **Settings → TikTok**, configure the COMMENT and DIRECT_MESSAGE
   webhooks, then connect the TikTok Business Account separately for each
   GarmentOS brand.
7. Test one real DM and one real comment. They remain human-managed in the
   Support Inbox by default. After DM send/read permissions are verified, set
   `TIKTOK_DM_AUTOREPLY_ENABLED=1`, then enable the chatbot only for each tested
   brand in **Settings → TikTok**.

Public TikTok comments intentionally remain manual so the order chatbot cannot
ask for an address, phone number, or payment details in a public thread. Human
